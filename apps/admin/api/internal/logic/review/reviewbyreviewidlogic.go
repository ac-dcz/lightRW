...
func (l *ReviewByReviewIdLogic) ReviewByReviewId(req *types.ReviewByReviewIdReq) (resp *types.ReviewResp, err error) {
	if items, err := l.svcCtx.EsReviewModel.FindByReviewId(l.ctx, req.ReviewId, req.Page, req.Size); err != nil {
		l.Errorf("find by review id error: %v", err)
